...
export default function TableauDashboard(): JSX.Element {
  ...
  // Handle drag start from data fields
  ...
  // Handle drop on chart area
  const handleChartDrop = useCallback(
    (e: React.DragEvent<HTMLDivElement>, chartId: string): void => {
      e.preventDefault();
  ...
  const renderChart = (item: DashboardItem): JSX.Element => {
    ...
    switch (component) {
      ...
      default:
        return (
          <div className="p-4 text-gray-500">Chart type not supported</div>
        );
    }
  };
          ...
          <div className="space-y-1">
            {fieldTypes.dimensions.map((field: string) => (
              <div
                key={field}
                draggable
                onDragStart={() => handleFieldDragStart(field, "dimensions")}
                className="p-2 bg-blue-50 border border-blue-200 rounded cursor-move hover:bg-blue-100 transition-colors text-sm"
              >
          ...
          <div className="space-y-1">
            {fieldTypes.measures.map((field: string) => (
              <div
                key={field}
                draggable
                onDragStart={() => handleFieldDragStart(field, "measures")}
                className="p-2 bg-green-50 border border-green-200 rounded cursor-move hover:bg-green-100 transition-colors text-sm"
              >
                🔢 {field.charAt(0).toUpperCase() + field.slice(1)}
              </div>
            ))}
            ...
            {chartTypes.map((chart: ChartType) => {
              ...
              return (
                <button
                  ...
                  className="p-2 border border-gray-200 rounded hover:bg-gray-50 transition-colors flex flex-col items-center text-xs"
                >
          ...
          {dashboardItems.length === 0 ? (
            <div className="flex items-center justify-center h-full">
              <div className="text-center text-gray-500">
                ...
                <p className="text-sm">
                  Add charts from the left panel and drag fields to customize
                  them
                </p>
              </div>
            </div>
              ...
              {dashboardItems.map((item: DashboardItem) => (
                ...
                >
                  ...
                  <div className="mt-2 text-xs text-gray-500">
                    X: {item.config.xAxis} | Y: {item.config.yAxis}
                  </div>
                </div>
              ))}
            </div>
